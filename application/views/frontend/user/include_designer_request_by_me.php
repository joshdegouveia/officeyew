<div class="orders-container">
    ...
    <table class="table table-hover custom_developer_table">
        ...
        <tbody>
            <?php if (count($la_designer_request_data) == 0) { ?>
                <tr>
                    <td colspan="4" class="no_data_row">No request submitted!</td>
                </tr>
                <?php
            }
            foreach ($la_designer_request_data as $row) {
//                echo "<pre>";
//                print_r($row);
                $status = $row->status;
                ...
                $viewBtnId = "dRequestByMe__$row->id" . "__" . $row->user_id;
                ?>

                <tr>
    <!--                    <td class="product_name">
                        <a class="display_block" href="<?php echo base_url(); ?>" target="_blank"> 
                    <?php // echo ucwords($user_detail->first_name." ".$user_detail->last_name) ?>
                        </a>
                    </td>-->
                    ...
                    <td class="payment_status processing designer_req_by_me designer_req_details" id="<?= $viewBtnId ?>">View Details</td>
                </tr>
            <?php }
            ?>
    ...
    <?php
    if ($la_myDesignerServiceRequest_count > ITEM_LIST) {
        $totalPage = (($la_myDesignerServiceRequest_count % ITEM_LIST) == 0) ? intval($la_myDesignerServiceRequest_count / ITEM_LIST) : (intval($la_myDesignerServiceRequest_count / ITEM_LIST) + 1);
            ...
            <ul class="pagination justify-content-center">
                ...
                if ($currentPage > 1) {
                    ...
                    <li class="page-item prev">
                        <a class="page-link" id="<?= ($currentPage - 1) ?>_service_request_pre" >
                ...
                for ($i = 1; $i <= $totalPage; $i++) {
                    if ($currentPage == $i) {
                        ?>
                        <li class="page-item activetab"><a class="page-link" id="<?= $i ?>_service_request" ><?= $i; ?></a></li>
                        <?php
                    } else {
                        ?>
                        <li class="page-item"><a class="page-link" id="<?= $i ?>_service_request" ><?= $i; ?></a></li>
                        <?php
                    }
                }
